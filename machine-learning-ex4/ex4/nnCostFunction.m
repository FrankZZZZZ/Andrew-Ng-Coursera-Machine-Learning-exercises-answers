...
Theta1 = reshape(nn_params(1:hidden_layer_size * (input_layer_size + 1)), ...
                 hidden_layer_size, (input_layer_size + 1)); % hidden_layer_size x input_layer_size+1
...
for i = 1 : m
  ...
  temp = Theta1 * [1 X(i, :)]';
  % a2
%  temp = sigmoid(temp); % size: hidden_layer_size x 1
  % z3
  temp = Theta2 * [1; sigmoid(temp)]; 
  % predicted 
%  h = sigmoid(temp); % size: num_labels x 1
  yi = zeros(1, num_labels);
  yi(y(i)) = 1; % size: 1 x num_labels
  J += - 1 / m * (yi * log(sigmoid(temp)) + (1 - yi) * log(1 - sigmoid(temp)) );
endfor
...
for t = 1 : m
  ...
  delta_2 = Theta2' * delta_3 .* [0; sigmoidGradient(z2)]; 
        % hidden_layer_size + 1 x 1 ; 
  Theta1_grad += delta_2(2:end) * ([1 a1]);
  Theta2_grad += delta_3 * [1 (a2)'];
endfor
